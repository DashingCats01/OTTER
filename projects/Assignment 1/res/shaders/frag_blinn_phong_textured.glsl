...
uniform bool  u_mode1;
uniform bool  u_mode2;
uniform bool  u_mode3;
uniform bool  u_mode4;
uniform bool  u_mode5;
...
void main() {
	...
	// inPos - world_pos
	vec3 V = normalize(u_LightPos - inPos);

	vec3 diffuseOut = (diffuse * textureColor.rgb) / (dist*dist);


	if (u_mode1 == true)
	{
		frag_color = vec4(inColor, textureColor.rgb);
	}
	if (u_mode2 == true)
	{
		vec3 ambient = u_AmbientLightStrength * u_LightCol;
	...
	if (u_mode3 == true)
	{
		...
		vec3 result = ((specular * attenuation) * inColor * textureColor.rgb);
	...
	if (u_mode5 == true)
	{
		diffuseOut = floor(diffuseOut*50) * (20.0/50);

		float edge = (dot(V, lightDir) < 0.20) ?0.0 : 1.0;
		vec3 result = (diffuseOut*edge) + (ambient + diffuse + specular) * attenuation;
