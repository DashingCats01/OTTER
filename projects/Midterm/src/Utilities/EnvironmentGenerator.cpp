	...
	for (int i = 0; i < _objectsToSpawn.size(); i++)
	{
		std::vector<GameObject> temp;
		{
			...
			for (int j = 0; j < _numToSpawn[i]; j++)
			{
				...
				temp[j].get<Transform>().SetLocalRotation(Util::GetRandomNumberBetween(glm::vec3(3.f, 90.f, 0.f), glm::vec3(3.f, 180.f, 360.0f)));
				temp[j].get<Transform>().SetLocalScale(0.50f,0.50f,0.50f);
			}
		}
