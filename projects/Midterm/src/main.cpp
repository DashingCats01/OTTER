...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		BackendHandler::imGuiCallbacks.push_back([&]() {
			...
			if (ImGui::CollapsingHeader("Effect controls"))
			{
				...
				if (activeEffect == 0)
				{
					...
					NoEffect* temp = (NoEffect*)effects[activeEffect];
				}
				if (activeEffect == 1)
				{
		...
		Texture2D::sptr zebra = Texture2D::LoadFromFile("images/Zebra.jpg");
		Texture2D::sptr snow = Texture2D::LoadFromFile("images/snow.jpg");
		Texture2D::sptr dagger1 = Texture2D::LoadFromFile("images/Dagger_Texture.png");
		Texture2D::sptr dagger2 = Texture2D::LoadFromFile("images/Dagger_Texture2.png");
		Texture2D::sptr eye = Texture2D::LoadFromFile("images/Eye.png");
		...
		ShaderMaterial::sptr dagger1mat = ShaderMaterial::Create();
		dagger1mat->Shader = shader;
		dagger1mat->Set("s_Diffuse", dagger1);
		dagger1mat->Set("s_Specular", specular);
		dagger1mat->Set("u_Shininess", 8.0f);
		dagger1mat->Set("u_TextureMix", 0.5f);

		ShaderMaterial::sptr dagger2mat = ShaderMaterial::Create();
		dagger2mat->Shader = shader;
		dagger2mat->Set("s_Diffuse", dagger2);
		dagger2mat->Set("s_Specular", specular);
		dagger2mat->Set("u_Shininess", 8.0f);
		dagger2mat->Set("u_TextureMix", 0.5f);
		...
		ShaderMaterial::sptr zebramat = ShaderMaterial::Create();
		zebramat->Shader = shader;
		zebramat->Set("s_Diffuse", zebra);
		zebramat->Set("s_Specular", specular);
		zebramat->Set("u_Shininess", 8.0f);
		zebramat->Set("u_TextureMix", 0.5f);

		ShaderMaterial::sptr eyemat = ShaderMaterial::Create();
		eyemat->Shader = shader;
		eyemat->Set("s_Diffuse", eye);
		eyemat->Set("s_Specular", specular);
		eyemat->Set("u_Shininess", 8.0f);
		eyemat->Set("u_TextureMix", 0.5f);
		...
		GameObject obj3 = scene->CreateEntity("Dagger");
		{
			...
			obj3.emplace<RendererComponent>().SetMesh(vao).SetMaterial(dagger1mat);
		...
		// Lamps : https://sketchfab.com/3d-models/lamp-low-poly-ada2d28e526e48faabf046a72e4c4480
		...
		// Bear : https://sketchfab.com/3d-models/we-bare-bears-low-poly-4a3742fffa604722b2ede238f88192b5#download
		GameObject obj5 = scene->CreateEntity("Bear");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/bear.obj");
			obj5.emplace<RendererComponent>().SetMesh(vao).SetMaterial(zebramat);
			obj5.get<Transform>().SetLocalPosition(0.0f, -7.0f, 1.0f);
		...
		GameObject obj6 = scene->CreateEntity("Eye");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/Eye.obj");
			obj6.emplace<RendererComponent>().SetMesh(vao).SetMaterial(eyemat);
			obj6.get<Transform>().SetLocalPosition(0.0f, 7.0f, 1.0f);
			obj6.get<Transform>().SetLocalRotation(90.0f, 0.0f, 0.0f);
			obj6.get<Transform>().SetLocalScale(2.0f, 2.0f, 2.0f);
		...
		EnvironmentGenerator::AddObjectToGeneration("models/Dagger.obj", dagger2mat, 20,
