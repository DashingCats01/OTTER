		...
		std::vector<GameObject> temp;
		{
			...
			for (int j = 0; j < _numToSpawn[i]; j++)
			{
				...
				temp[j].get<Transform>().SetLocalPosition(glm::vec3(Util::GetRandomNumberBetween(_spawnFromAll[i],
					_spawnToAll[i], _avoidFromAll[i], _avoidToAll[i]), 1.0f));
				temp[j].get<Transform>().SetLocalScale(0.5f, 0.5f, 0.5f);
				temp[j].get<Transform>().SetLocalRotation(Util::GetRandomNumberBetween(glm::vec3(0.f, 0.f, 0.f), glm::vec3(0.f, 90.f, 360.0f)));
			}
		}
